...
def messages(self):
  ...
  self.maxMenuVal = 4
  self.menuOptions.clear()
  self.menuOptions.append("1. New message")
  self.menuOptions.append("2. Display messages")
  self.menuOptions.append("3. Back")
  self.displayPage()
  
  choice = getNumber(self.breadcrumb, self.minMenuVal, self.maxMenuVal)
  if choice == 1:
    tmpNewMessage = Messages()
    self.newMessage()
  elif choice == 2:
...
def newMessage(self, tmpNewMessage):

  self.newMessage = tmpNewMessage.getNewMessage()
  self.pageName = "New Message"
  self.breadcrumb = "Dashboard / New Message>"
  self.minMenuVal = 1
  self.maxMenuVal = 2

  self.menuOptions.clear()
  self.menuOptions.append("1. Message: " + self.new)
  self.menuOptions.append("2. Submit new message to database")
  self.menuOptions.append("3. Back")
  self.displayPage()

  choice = getNumber(self.breadcrumb, self.minMenuVal, self.maxMenuVal)
  if choice == 1:
    choice = getString("Message: ", minLen=1, maxLen=50)
    self.newMessage = choice
    tmpNewMessage.newMessage = choice
    self.newMessage(tmpNewMessage)
  elif choice == 2:
    if getYesNo("Are you sure you want to submit this message?") == "y":
      db.addMessageToDB(tmpNewMessage)
      self.displayDashboard()
    else:
      self.newMessage(tmpNewMessage)
